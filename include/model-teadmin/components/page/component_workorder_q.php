...
<div data-role="content">
 ...
 <div style="text-align:right; float:right;">
  ...
  <div style="margin-top:60px;  padding-bottom:5px;">  รายการใบแจ้งซ่อมซึ่งถูกกำหนดโดย Supervisor ของท่าน </div>
...
<!-- tbl sparepart -->
  <table  style="width:100%;" id="tlb">
        <tr style="height:40px; vertical-align:middle; text-align:center; font-weight:bold; background-color:#CFCFCF;">
          ...
          <?php     
   if($D=='ALL' ){
	   $stm="SELECT * FROM `tpm_jobreq` WHERE `MM` = '$MM'  and `YYYY` ='$YYYY'  and  `plant` like  '$plant' AND `jobto_user` LIKE  '".$_SESSION['uname']."' ORDER BY  `tpm_jobreq`.`jobreq_id` DESC  ";  
   }else if ($D != 'ALL') {
	    $stm="SELECT * FROM `tpm_jobreq` WHERE `MM` ='$MM'   and `YYYY` = '$YYYY'  and  `DD` = '$D'  and  `plant` like  '$plant'  AND `jobto_user` LIKE  '".$_SESSION['uname']."'   ORDER BY  `tpm_jobreq`.`jobreq_id` DESC ";
   }
   $job=mysql_query($stm);
            $i=1 ;         
   if ($job != ''){
   ...
   <?php  while ($arrjob=mysql_fetch_array($job)){   ?>
    ...
    <a href="#" data-rel="back" class="ui-btn ui-corner-all ui-shadow ui-btn-a ui-icon-delete ui-btn-icon-notext ui-btn-right">ปิด</a><img class="popphoto" src="http://lionproduction.sli/pdmis/tpm/images/WORKORDER/<?php echo $arrjob['jobreq_imgname']; ?>" style="max-height:512px;" alt="<?php echo $arrjob['jobreq_imgname']; ?>">
            ...
            <td> <div style="text-align:center;"><?php echo $arrjob['DD']; ?>/<?php echo $arrjob['MM']; ?></div></td>
           ...
           <a href="../../include/model-teadmin/components/page/subpast/popup_edit_workorder.php?JRID=<?php echo $arrjob['JRID']; ?>" data-ajax="false" id="editForm" data-transition="pop" class="ui-btn ui-shadow ui-corner-all ui-icon-edit ui-btn-icon-notext " style="display: inline-flex;">Edit list</a>
